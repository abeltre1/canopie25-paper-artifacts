...
def _facts_shape(facts, scheduler_name: str) -> tuple[int, int, str] | None:
    """The node shape straight from THIS run's cluster probe (fresher than any
    card file): (gpus_per_node, gpu_vram_gb, provenance), or None when the
    probe didn't run / the inventory carries no GPU shape."""
    if facts is None:
        return None
    from boxy import site

    inv = facts.inventory(scheduler_name)
...
def _app_site_args(args, card, scheduler, scheduler_name: str, target: str = "",
                   host: str = "", cfacts=None) -> list[str]:
    """Site directives for an app job, resolved the same way the serve path does:
    account (config/env, else myaccounts probed on the cluster), partition (explicit
    or auto-ranked from the cluster), time (flag > card > config default), and the
    Slurm license default. Explicit flags always win. `cfacts` (the one-shot
    cluster probe's report) replaces the per-item ssh round-trips."""
    ...
    if target and site.partition_mode(part) in ("auto", "all"):
        if cfacts is not None:
            out = cfacts.partitions_out(scheduler_name)
        else:
            rc, out = remote.ssh_capture(target, site.remote_partition_probe(scheduler_name), timeout=20)
            out = out if rc == 0 else ""
        value, pwhy = site.rank_remote_partitions(out, scheduler_name, need_gpu)
        if value:
...
def _looks_like_ompi_ucx_failure(text: str) -> bool:
    ...
    OpenMPI has no ucx (field: clusterc)."""
    low = text.lower()
...
def _app_agentless_ssh(args, card, target: str) -> int:
    ...
    completion and print the log — so `boxy app osu-benchmarks --ssh clusterd`
    ...
    # ONE composite probe for the app flow's cluster facts too (scheduler,
    # partitions, account, $HOME) — the same round-trip saver as serve.
    cfacts = _cluster_facts(target, host)

    scheduler_name = getattr(args, "scheduler", None)
    if scheduler_name not in ("slurm", "flux"):
        if cfacts is not None:
            avail = cfacts.sched_avail()
        else:
    ...
    if cfacts is not None:
        print(_cluster_banner(host, cfacts, scheduler_name))

    site_args = _app_site_args(args, card, scheduler, scheduler_name, target=target, host=host,
                               cfacts=cfacts)

    rhome = cfacts.home() if cfacts is not None else ""
    if not rhome:
    ...
    # the site filter 403s — the same fix that made image pulls work (field: clustera).
    ...
    # same auto-recovery as the serve path (field: clusterd).
    ...
    if card.service:
        ...
        while _time.time() < deadline:
            ...
            # clusterc — READY never fired because the output didn't START with
...
def _is_license_error(text: str) -> bool:
    ...
    specification') — the shipped site.license default (sitescratch:1, a clustera-ism)
    doesn't exist on every cluster (field: clusterd). The self-heal drops the
...
def _looks_like_proxy_failure(text: str) -> bool:
    ...
    site proxy (field: clusterb vs the proxy clustera needs) should retry WITHOUT it."""
    low = (text or "").lower()
...
def _gres_fallback_forms(gtypes: list[str]) -> list[tuple[str, str]]:
    ...
    configuration is not available' from a wrong pinned type — field: clusterd/clustera).
    ...
    clusterd REQUIRES the type and rejects all untyped spellings, and a cluster
    ...
    # partition reports (null); field: clusterd) rejects every gres/gpus spelling,
...
def _pick_account(args, where: str = "", rows=None) -> None:
    ...
    ('never'), an account is already set, or a pin ($ACCOUNT_ID / site.account) applies
    ...
    if getattr(args, "account", None) or _pick_mode(args) == "never":
        return
    explicit = os.environ.get("ACCOUNT_ID", "").strip() or config.get_str("site.account").strip()
    if rows is None:
...
def _pick_remote_account(args, rows, host: str, target: str) -> tuple[str | None, str]:
    """Choose the account from a cluster-probed (account_id, label) list for an --ssh
    ...
    first-pick with the legacy `myaccounts on <host>; also: …` note, so batch/CI and
    ...
    if len(accounts) > 1 and picker.is_interactive(_pick_mode(args)):
        pick, note = picker.choose_account(rows, remembered=picker.recall(target),
                                           ...
                                           source=f"myaccounts on {host}")
        if pick:
            return pick, note
    extra = f"; also: {', '.join(accounts[1:3])}" if len(accounts) > 1 else ""
    return accounts[0], f"myaccounts on {host}{extra}"
...
def _stage_agentless_ca(target: str, host: str, rdir: str, dryrun: bool = False) -> str | None:
    ...
    clustera). We push the bundle here and hand the path to deploy.set_agentless_ca().
...
def _cluster_facts(target: str, host: str, partition: str = "", store_saved: str = ""):
    """ONE composite round-trip for every static cluster fact (scheduler
    liveness, GRES/accel signals, node inventory, partitions, account, model
    store, $HOME/hostname). Returns site.ClusterFacts, or None when the report
    came back unusable — every consumer then falls back to its individual
    probe, so a truncated report can never half-decide."""
    import time as _time

    from boxy import remote, site

    t0 = _time.time()
    rc, out = remote.ssh_capture(target, site.cluster_probe(partition, store_saved), timeout=60)
    facts = site.parse_cluster_report(out if rc == 0 else "", partition=partition,
                                      seconds=_time.time() - t0)
    if facts is None:
        print(f"boxy: warning: the one-shot cluster probe returned nothing usable on {host} — "
              f"falling back to individual probes", file=sys.stderr)
    return facts


def _cluster_banner(host: str, facts, scheduler_name: str) -> str:
    """The compact cluster-facts line printed when the composite probe ran —
    the at-a-glance summary above the per-decision `auto:` lines."""
    from boxy import site

    bits = [scheduler_name or "no scheduler"]
    accel, _why = facts.accel()
    if accel and accel != "nogpu":
        bits.append(accel)
    inv = facts.inventory(scheduler_name)
    if inv.get("gpus_per_node"):
        vram = inv.get("gpu_vram_gb") or site.gpu_vram_from_type(inv.get("gpu_type", ""))[0]
        shape = f"{inv['gpus_per_node']}x{inv.get('gpu_type') or 'gpu'}"
        if vram:
            shape += f" {vram}GB/node"
        bits.append(shape)
    if inv.get("total_nodes"):
        bits.append(f"{inv.get('total_gpu_nodes') or inv['total_nodes']} GPU nodes"
                    if inv.get("total_gpu_nodes") else f"{inv['total_nodes']} nodes")
    accounts = site.parse_accounts(facts.account_out())
    if accounts:
        bits.append(f"account {accounts[0]}")
    spath, _free, _swhy = site.pick_model_store(facts.store_out(),
                                                config.get_int("storage.min_free_gb"))
    if spath:
        bits.append(f"store {spath}")
    return f"### {host}: " + " · ".join(bits) + f"  (1 probe, {facts.seconds:.1f}s)"


def _write_system_card_from_facts(host: str, facts, scheduler_name: str, accel: str) -> None:
    """Write-through: the composite probe's facts refresh the per-cluster
    system card (the artifact the geometry solver reads), so every serve keeps
    it current. Only files carrying the generated-by header are overwritten —
    a hand-edited card stays untouched (this run still uses the fresh facts).
    Best-effort: a write problem must never block a serve."""
    from boxy import cards, site

    try:
        inv = facts.inventory(scheduler_name)
        if not inv or not inv.get("total_nodes"):
            return
        if inv.get("gpus_per_node") and not site.gpu_vram_from_type(inv.get("gpu_type", ""))[0]:
            htype, hvram, _hcount = facts.gpu_hw()
            if htype:
        ...
        runtimes = [r for r in facts.runtimes() if r in ("podman", "apptainer", "docker", "ch-run")]
        storage = []
        for line in (facts.store_out() or "").splitlines():
            if not line.strip():
                continue
            path, free_gb, _w = site.pick_model_store(line, 0)
            if path:
                storage.append(f"{path}  {free_gb} GB free")
        text = site.render_system_card(host, scheduler_name, accel or "none",
                                       runtimes[0] if runtimes else "", inv, storage)
        dest = cards._user_systems_dir() / f"{host}.toml"
        if dest.exists() and "generated by `boxy generate system`" not in dest.read_text()[:400]:
            return  # hand-edited card — never clobber it
        dest.parent.mkdir(parents=True, exist_ok=True)
        dest.write_text(text)
    except Exception:  # noqa: BLE001 — a card refresh must never break a serve
        pass


def _remote_model_store(target: str, host: str, rdir: str, dryrun: bool = False,
                        probe_out: str | None = None, saved: str | None = None) -> str:
    """Where multi-GB model downloads live on the CLUSTER: a big SHARED scratch
    FS (sitescratch class), never the $HOME quota (field: a 100GB+ model exhausted
    the user's home quota mid-serve). storage.model_dir/BOXY_MODEL_DIR pins an
    exact path; otherwise the login node is probed once ($SCRATCH, /sitescratch,
    ...
    `probe_out` (from the one-shot cluster probe) skips the dedicated ssh
    round-trip; `saved` skips re-reading the sticky file."""
    ...
    if probe_out is None:
        ...
        out = probe_out
    picked, free_gb, why = site.pick_model_store(out, config.get_int("storage.min_free_gb"))
    if picked:
...
def _hf_curl_script(repo: str, stage: str, image: str, *, proxy: str = "", cacert: str = "",
                    token: str = "", force: bool = False) -> str:
    """The download script `boxy pull --ssh` runs detached on the login node.

    Plain curl on the HOST — the same binary, trust store, and network path the
    egress probe verified — because that combination is the only one with an
    observed 200 against this cluster. No container, no Python TLS stack, no
    certifi anywhere in the download; the serving image is pre-pulled but only
    warmed, never depended on.

    Resume shape: each file downloads to <name>.part with `curl -C -` and is
    renamed only when complete; files already renamed are skipped. A rerun
    therefore continues instead of restarting, and a half-written shard can
    never be mistaken for a finished one (the .part suffix also keeps it out of
    the status probe's shard count)."""
    q = shlex.quote
    curl = "curl -sfL --retry 5 --retry-delay 15"
    if cacert:
        curl += f" --cacert {q(cacert)}"
    # SECRETS NEVER ON ARGV. A shared login node lets ANY user read
    # /proc/<pid>/cmdline, and this curl runs for the length of a multi-GB
    # download — so `-H "Authorization: Bearer $HF_TOKEN"` and a proxy URL with
    # inline credentials were readable by everyone on the node for hours. Both
    # go into a 0600 curl config file instead (curl -K); the argv carries only
    # its path. The heredoc keeps the values off argv while WRITING it too (cat
    # reads stdin), and a trap removes the file on every exit path.
    auth = ""
    conf_block: list[str] = []
    if token or proxy:
        curl += ' -K "$CURLRC"'
        conf_block = [
            ...
            'cat > "$CURLRC" <<EOF_BOXY_CURLRC',
        ]
        if token:
            conf_block.append('header = "Authorization: Bearer $HF_TOKEN"')
        if proxy:
            conf_block.append('proxy = "$BOXY_PROXY_URL"')
        conf_block.append("EOF_BOXY_CURLRC")
    # The LISTING deliberately drops -f: with -f curl throws the body AND the
    # status away, so a gated repo, a blocked API path, a rate limit and a DNS
    # failure all produced the same six unactionable words ("could not list the
    # repo files"). Keep the code and turn it into the actual remedy.
    list_opts = ""
    if cacert:
        list_opts += f" --cacert {q(cacert)}"
    if token or proxy:
        list_opts += ' -K "$CURLRC"'
    # -L IS REQUIRED: the Hub legitimately redirects some API paths, and a
    # captive/intercepting proxy answers with one too. Without it the listing
    # fails on a 3xx that the previous `curl -sfL` handled — a regression this
    # rewrite introduced while dropping -f. -w reports the FINAL status.
    list_curl = f"curl -sSL --retry 3 --retry-delay 5{list_opts}"
    lines = [
        "#!/bin/sh",
        "# generated by boxy pull — host-curl download; safe to re-run (it resumes)",
        "set -u",
        f"REPO={q(repo)}",
        f"STAGE={q(stage)}",
    ]
    if token:
        lines.append(f"HF_TOKEN={q(token)}")
    if proxy:
        lines.append(f"BOXY_PROXY_URL={q(proxy)}")
    lines += conf_block
    if force:
        lines.append('rm -rf "$STAGE"')
    lines += [
        'mkdir -p "$STAGE"',
        # a FAILURE MARKER, so the status probe can tell 'died' from 'running'
        ...
        'printf "%s" "${code#* }" > "$STAGE/.boxy-repo.url"',   # where it really landed
        'code=${code%% *}',
        '[ -n "$code" ] || code=000',
        'case "$code" in',
        '  200) : ;;',
        '  401|403) _fail "HuggingFace refused the repo listing (HTTP $code). '
        '$REPO is GATED or the token was rejected: accept the license at '
        'https://huggingface.co/$REPO with the account owning your token, export HF_TOKEN, '
        'and re-run. (A network policy that blocks the Hub also shows as 403.)" ;;',
        '  404) _fail "$REPO does not exist on HuggingFace (HTTP 404) — check the id for a typo, '
        'or your token cannot see a private repo." ;;',
        '  429) _fail "HuggingFace rate-limited the listing (HTTP 429). Wait a few minutes and '
        're-run — the download resumes where it stopped." ;;',
        '  000) _fail "could not reach huggingface.co at all from this node: '
        '$(tr -d "\\r" < "$STAGE/.boxy-repo.err" | tail -2). DNS/TLS/proxy, not the model. '
        'Try: curl -v https://huggingface.co/api/models/$REPO" ;;',
        '  *) _fail "HuggingFace returned HTTP $code for the repo listing: '
        '$(head -c 200 "$STAGE/.boxy-repo.json" 2>/dev/null)" ;;',
        'esac',
        ("files=$(tr ',' '\\n' < \"$STAGE/.boxy-repo.json\" "
         "| grep -o '\"rfilename\": *\"[^\"]*\"' | cut -d'\"' -f4)"),
        ('[ -n "$files" ] || _fail "the repo listing did not come back as HuggingFace JSON '
         '(final HTTP $code from $(cat \"$STAGE/.boxy-repo.url\" 2>/dev/null)). This is what '
         'a captive portal or an intercepting proxy looks like: the request was answered by '
         'something else. First bytes: $(head -c 200 \"$STAGE/.boxy-repo.json\" | tr -d \"\\n\"). '
         'If that is HTML, this node needs the site proxy (--proxy URL) or the bundle path."'),
        "fail=0",
        "for f in $files; do",
        '  case "$f" in original/*|*.pth|consolidated*|*.gguf) continue;; esac',
        '  [ -f "$STAGE/$f" ] && continue',
        '  echo "fetch $f"',
        (f'  {curl} -C - --create-dirs{auth} -o "$STAGE/$f.part" '
         '"https://huggingface.co/$REPO/resolve/main/$f" '
         '&& mv "$STAGE/$f.part" "$STAGE/$f" || { echo "boxy-pull: FAILED $f"; fail=1; }'),
        "done",
        '[ "$fail" = 0 ] && touch "$STAGE/.boxy-pull-complete" && echo "boxy-pull: complete"',
        '[ "$fail" = 0 ] || printf "%s\\n" "some files failed to download — re-run to resume" '
        '> "$STAGE/.boxy-pull-failed"',
        'exit "$fail"',
    ]
    return "\n".join(lines) + "\n"
...
def _pull_agentless_ssh(args, target: str) -> int:
    ...
    # FAILED is checked BEFORE liveness: the log's mtime cannot tell 'downloading'
    # from 'crashed 30 seconds ago', and reporting RUNNING for a dead script sent
    ...
    if state == "FAILED" and not args.force:
        why = kv.get("WHY") or "see the log"
        print(f"pull FAILED on {host} after {got_gb}{of_gb}, {got_shards}{of_shards} shards",
              file=sys.stderr)
    ...
    print(f"  auto: image: {image} (pre-pulled for the serve while the download runs — "
          "--image to override)")

    # THE DOWNLOAD RUNS AS PLAIN CURL ON THE LOGIN NODE — the exact tool, trust
    # store, and network path the egress probe verifies. Field: SIX rounds of
    # in-container CERTIFICATE_VERIFY_FAILED (laptop proxy injected; CA never
    # staged; CA for the wrong network; certifi vs the node's ca-trust) while
    # the login node's own curl answered 200 every single time. The fix is to
    # stop translating that working configuration into a different TLS stack
    # and just USE it: no container, no Python, no certifi anywhere in the
    # download. The serving image is still pre-pulled, but only warmed — the
    # pull never depends on it.
    rc_e, _eout = remote.ssh_capture(
        target, "curl -sIf https://huggingface.co -o /dev/null --max-time 12", timeout=20)
    if rc_e == 0:
        print(f"  auto: egress: huggingface.co verifies directly from {host} — downloading "
              "with the login node's own curl, exactly as the probe just did")
        proxy_url, cacert = "", ""
    else:
        penv = remote.remote_proxy_env()
        proxy_url = (penv.get("https_proxy") or penv.get("HTTPS_PROXY")
                     or penv.get("http_proxy") or "")
        cacert = _stage_agentless_ca(target, host, rdir, dryrun=args.dryrun) or ""
        print(f"  auto: egress: direct HF probe failed from {host} — curl will go through "
              f"{redact.redact_url_credentials(proxy_url) or 'no proxy (none known)'}"
              + (" with the staged site CA" if cacert else ""))
    tok = os.environ.get("HF_TOKEN") or os.environ.get("HUGGING_FACE_HUB_TOKEN") or ""
    script = _hf_curl_script(repo, stage, image, proxy=proxy_url, cacert=cacert,
                             token=tok, force=args.force)
    script_remote = f"{rdir}/pull-{slug}.sh"
    ...
    launch = (f"mkdir -p {q(rdir)}; chmod 600 {q(script_remote)}; "
              f"[ -f {q(log_remote)} ] && mv -f {q(log_remote)} {q(log_remote)}.prev; "
              f"setsid nohup sh {q(script_remote)} >> {q(log_remote)} 2>&1 < /dev/null & "
              f"echo $! > {q(pid_remote)}; cat {q(pid_remote)}")
    if args.dryrun:
        print(f"### would launch on {host} (detached, survives disconnect): host-curl download "
              f"of {repo} -> {stage} via {script_remote}")
        return 0
    if remote.push_file(target, script_remote, script) != 0:
        print(f"boxy: could not stage the pull script on {host}", file=sys.stderr)
        return 1
...
def _serve_agentless_ssh(args, target: str) -> int:
    ...
    # 0) ONE composite probe: every static cluster fact in a single round-trip
    #    (scheduler liveness, GRES/accel signals, node inventory, partitions,
    #    account, model store, $HOME). The individual probes below remain as
    #    the fallback when the report comes back unusable.
    part_scope = (getattr(args, "partition", None) or "").strip()
    if site.partition_mode(part_scope or None) == "set":
        part_scope = part_scope or config.get_str("site.partition").strip()
    else:
        part_scope = ""
    _sticky_store = jobs._dir() / f".model-store-{jobs.cluster_id(host)}"
    store_saved = _sticky_store.read_text().strip() if _sticky_store.exists() else ""
    cfacts = _cluster_facts(target, host, partition=part_scope, store_saved=store_saved)
    ...
    if not getattr(args, "accelerator", None):
        ...
        if probed:
            ...
            if pwhy:
                where = f"cluster probe: {pwhy}"
            else:
                where = (f"partition {part}'s GPU inventory" if part
                         else f"the GPU stack on {host}'s login node")
            print(f"  auto: accelerator: {probed} (via {where})")

    # 1c) write-through: the probe's facts refresh the system card (generated
    #     cards only; hand-edited ones stay). Skipped on --dryrun like every
    #     other laptop-side state write.
    if cfacts is not None and not getattr(args, "dryrun", False):
        _write_system_card_from_facts(host, cfacts, scheduler_name,
                                      getattr(args, "accelerator", "") or "")
    ...
    #    cluster's node shape — THIS run's probed inventory when available,
    #    else its system card — so a fat-VRAM system uses fewer GPUs and an
    ...
    # 5) site over ssh: account (config/env, else myaccounts on the cluster), partition
    #    (sinfo/flux queue on the cluster), time (config default). Explicit flags win.
    site_args = list(location.scheduler_args)
    acct, awhy = site.resolve_account(getattr(args, "account", None))
    if not acct:
        # probe myaccounts on the cluster and pick laptop-side — works against an
    ...
    if part and site.partition_mode(part) not in ("off",):
        if scheduler_name == "flux" and "," in part:
            part = part.split(",")[0].strip()
        site_args.append(scheduler.site_directive("partition", part))
    # Slurm license(s): --license / config, e.g. #SBATCH --license=sitescratch:1
    if scheduler_name == "slurm":
        lic, lwhy = site.resolve_license(getattr(args, "license", None))
        if lic:
            site_args.append(scheduler.site_directive("license", lic))
    ...
    # clustera/clusterb/…). We DON'T proactively change a working cluster's directive;
    # if a site rejects it at submit (field: clusterd), the submit block below
    ...
    # The cache lives on the big SHARED SCRATCH FS (sitescratch class), never the
    # $HOME quota (field: a 100GB+ model exhausted the user's home quota).
    store_dir = rdir
    if not bundle and (engine_pull or _prestage_mode(args) != "never"):
        store_dir = _remote_model_store(
            target, host, rdir, dryrun=args.dryrun,
            probe_out=(cfacts.store_out() if cfacts is not None else None), saved=store_saved)
    ...
    #     pull trusts the site's TLS interceptor (field: clustera CERTIFICATE_VERIFY_
    ...
    # tunnels to the wrong host and readiness never confirms (field: clusterb,
    # 'server starting on cbnode1066' while the job ran on cbnode1321).
    if remote.push_file(target, script_remote, script_text) != 0:
        return 1
    ...
    # clusterd) and want --gres=gpu:[type:]N. Instead of telling the user to set an
    ...
    # (sitescratch:1 — clustera gates filesystems behind it) that other clusters don't
    # define ('Invalid license specification' — field: clusterd, surfaced after the
    ...
    if rc != 0:
        ...
        if "gres" in low or "generic resource" in low:
            ...
            print("boxy: hint: the account was rejected — pass --account <account_id> or export "
    ...
    # (field: clustera). An explicit LONGER --ready-timeout still wins.
    ...
    def _maybe_proxy_heal(tail: str) -> bool:
        """The job died reaching the FORWARDED proxy (field: clusterb can't reach
        the proxy clustera needs). Resubmit ONCE without the proxy — the node may hit
    ...
    try:
        while time.time() < deadline:
            ...
            if state == "DONE":
                ...
                # can extract the actual root cause (field: clusterb, only the
...
def _serve_submission(args, scheduler_name: str, profile, name_override: str | None = None,
    ...
    # probing — then fill account/partition/time from cards/myaccounts/env/sacctmgr.
    if not args.dryrun:
    ...
    # interactive ACCOUNT_ID picker (once, on the primary submission): several accounts +
    ...
    # --gres=gpu:[type:]N, not --gpus-per-node — field: clusterd): re-render with the
    ...
    try:
        while True:
            ...
            if endpoint:
                ...
                # startup complete." on clusterc5 but http://clusterc5:8000 was
...
def _inject_remote_site(args, target: str, raw_argv: list[str]) -> list[str]:
    ...
    rewrite the delegated argv:
      ...
      * account — append `--account <val>` (from config/env, else a `myaccounts`
    ...
    # ONE composite probe covers the scheduler + partition injections below.
    cfacts = _cluster_facts(target, host) if master_ok else None
    ...
    if mode in ("auto", "all"):
        ...
        if cfacts is not None:
            value, why = site.rank_remote_partitions(cfacts.partitions_out(scheduler),
                                                     scheduler, prefer_gpu)
        elif master_ok:
...
def cmd_generate_system(args: argparse.Namespace) -> int:
    ...
    # ONE composite probe: scheduler liveness, accel signals, runtime, node
    # inventory, storage ladder — every fact this command used to gather with
    # its own round-trip. The individual caps below remain the fallback.
    rc, creport = cap(site.cluster_probe(), timeout=90)
    cfacts = site.parse_cluster_report(creport if rc == 0 else "")
    if cfacts is not None:
        avail = cfacts.sched_avail()
    else:
        rc, avail = cap(site.remote_scheduler_probe())
        avail = avail if rc == 0 else ""
    scheduler, why = site.pick_scheduler(avail, None)
    if scheduler not in ("slurm", "flux"):
        if target:
    ...
    if cfacts is not None:
        accel, awhy = cfacts.accel()
        accel = "" if accel == "nogpu" else accel
        awhy = awhy or "cluster probe"
    else:
        rc, aout = cap(site.remote_accel_probe())
        accel, awhy = site.parse_remote_accel(aout if rc == 0 else ""), "the scheduler's GPU inventory"
    if accel:
        print(f"  auto: accelerator: {accel} (via {awhy})")
    if cfacts is not None:
        rtout = " ".join(cfacts.runtimes())
    else:
        rc, rtout = cap(site.runtime_probe())
        rtout = rtout if rc == 0 else ""
    ...
    if cfacts is not None:
        inv = cfacts.inventory(scheduler)
    ...
    if inv.get("gpus_per_node") and not site.gpu_vram_from_type(inv.get("gpu_type", ""))[0]:
        ...
        # 'unknown GPU type'. The composite report already carries this section.
        if cfacts is not None:
            htype, hvram, _hcount = cfacts.gpu_hw()
        else:
...
def _agentless_records() -> list[dict]:
    ...
    --ssh clustera` ran podman ps on the LOGIN node and showed nothing while the
...
def _scheduler_reachable(scheduler_obj) -> bool:
    ...
    clusterA flux record passed this check and boxy curl chased cbnode1025)."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--account", default=None, help="charge account/ACCOUNT_ID (default: config/env, "
    ...
    p.add_argument("--account", default=None,
                   help="account/bank for --scheduler jobs (Slurm --account, Flux --bank). "
                        "Omit to auto-discover (myaccounts/sacctmgr); with several accounts and a "
                        "TTY, boxy shows an interactive picker. $ACCOUNT_ID also bypasses the menu.")
    pick = p.add_mutually_exclusive_group()
    pick.add_argument("--pick-account", dest="pick_account", action="store_true", default=None,
                      help="force the interactive ACCOUNT_ID picker even when auto-discovery would "
    ...
    p.add_argument("--license", default=None,
                   help="Slurm license(s) as #SBATCH --license=VAL (e.g. sitescratch:1 or "
                        "sitescratch:1,pscratch:1), for sites that gate filesystems behind "
    ...
    p.add_argument("--scheduler-arg", action="append", default=[], dest="scheduler_args", metavar="FLAG",
                   help="extra raw scheduler flag for the job (repeatable), "
                        "e.g. --scheduler-arg=--license=sitescratch:1")
...
#   boxy serve M --scheduler slurm --account=acct --license=sitescratch:1
