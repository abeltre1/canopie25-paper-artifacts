...
def _hf_curl_script(repo: str, stage: str, image: str, *, proxy: str = "", cacert: str = "",
    ...
    if token or proxy:
        list_opts += ' -K "$CURLRC"'
    list_curl = f"curl -sS --retry 3 --retry-delay 5{list_opts}"
    lines = [
        "#!/bin/sh",
    ...
    lines += [
        ...
        (f'code=$({list_curl} -o "$STAGE/.boxy-repo.json" -w "%{{http_code}}" '
        ...
        '[ -n "$files" ] || _fail "the Hub listed no files for $REPO (HTTP 200 but an '
        'unexpected body: $(head -c 200 \"$STAGE/.boxy-repo.json\"))"',
        "fail=0",
        "for f in $files; do",
