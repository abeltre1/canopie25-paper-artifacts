...
def cluster_id(host: str) -> str:
    ...
    # a laptop asset tag like 'laptop' trims to 's' — a meaningless bucket; keep
...
def mark_endpoint_ready(path: str | Path, model: str = "") -> Path | None:
    ...
    routing issues (field report: the server was up on clusterc5 but the login-node
    probe to http://clusterc5:8000 went through the corporate proxy and never
