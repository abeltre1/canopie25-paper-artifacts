...
def remote_checks(run) -> list[Result]:
    ...
    # binaries exist. A Flux system with slurm-compat shims (field case: clusterb)
    # would otherwise read as "sbatch, flux" and mislead; here it reads "flux (live)".
    from boxy import site

    # ONE composite probe feeds the scheduler/account/partition checks below —
    # same round-trip saver as the serve path; per-check probes are the fallback.
    _, _creport = run(site.cluster_probe())
    _cfacts = site.parse_cluster_report(_creport)

    if _cfacts is not None:
        sched_out = _cfacts.sched_avail()
    else:
    ...
    # SAME probe (`myaccounts || sacctmgr`) the --ssh serve path uses and parse it,
    ...
    if _cfacts is not None:
        acct_out = _cfacts.account_out()
    else:
        rc_acct, acct_out = run(site.remote_account_probe())
        acct_out = acct_out if rc_acct == 0 else ""
    accounts = site.parse_accounts(acct_out)
    if accounts:
        ...
        out.append(Result("account discovery", WARN,
                          f"no account parsed from myaccounts/sacctmgr on the login node{detail}",
                          "pass --account <account_id> (or export BOXY_ACCOUNT); the scheduler's site default "
    ...
    if picked in ("slurm", "flux"):
        if _cfacts is not None:
            part_out = _cfacts.partitions_out(picked)
        else:
            rc_part, part_out = run(site.remote_partition_probe(picked))
            part_out = part_out if rc_part == 0 else ""
        value, _why = site.rank_remote_partitions(part_out, picked)
        if value:
