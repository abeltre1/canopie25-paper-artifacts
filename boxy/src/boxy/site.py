...
Account probe chain (first hit wins):
  ...
  2. $ACCOUNT_ID                                    (a session bypass for the picker)
  3. config site.account                     (BOXY_ACCOUNT / [site].account)
  4. site.account_command                    (default `myaccounts`, the site's charge-account ID lister)
...
are shim-testable (a bash `myaccounts`/`sacctmgr` on PATH). Partition/time come from
...
# An account/charge-account ID token: letters then >=4 digits (ab110003, AB110001) — the
# preferred shape; a bare 6-8 digit ID is accepted only when no letter-prefixed
# token exists anywhere. Real `myaccounts` rows carry BOTH (`... ab110001   103732
# system software ...` — the description starts with a numeric id), and search()
# order picks the account: 'user1' has no digits, so ab110001 is the first hit.
_ACCOUNT_RE = re.compile(r"\b([A-Za-z]{1,4}\d{4,})\b")
_BARE_ID_RE = re.compile(r"\b(\d{6,8})\b")

# Header/separator lines from the real myaccounts table (field sample, 2026-07):
#       User    Account                              Description     Parent
#   ---------- ---------- ------------------------------------ ----------
# plus charge-account ID-style headers and dashed rules: never mine these for tokens.
_HEADER_RE = re.compile(
    r"^\s*[-=+\s]+$"                      # dashed/blank separator rules
    r"|^\s*user\s+account\b"              # the real myaccounts header row
...
def parse_accounts(text: str) -> list[str]:
    ...
    Tolerant of the layouts seen in the field: a `myaccounts` TABLE (header row +
    data rows), a labelled line (`ACCOUNT_ID: ab110003 (Project)`), a bare list, and
    `sacctmgr -nP` single-column output. Header/separator lines are skipped;
    letter-prefixed IDs (ab110003) win; bare 6-8 digit IDs are the fallback
    ONLY when no letter-prefixed token exists anywhere."""
    prefixed: list[str] = []
    bare: list[str] = []
    seen_ci: set[str] = set()   # myaccounts's trailing "could be on Caps too: AB110001"
...
def _row_label(line: str, m: re.Match) -> str:
    """A short human label for one account row — the descriptive text `myaccounts`
    ...
    (`ab110001  103732 system software` -> `system software`); falls back to
    text before the id (labelled `ACCOUNT_ID: fy... (Project)` layouts)."""
...
def parse_account_rows(text: str) -> list[tuple[str, str]]:
    ...
    picker: [(account_id, label), ...], in order, de-duplicated. Same header-skip and
...
def discover_account_rows() -> list[tuple[str, str]]:
    """(account_id, label) rows from the configured site account command (default
    `myaccounts`), for the interactive picker. [] if the command is unset, missing,
...
def resolve_account(explicit: str | None) -> tuple[str | None, str]:
    ...
    why = ("no account discovered (myaccounts / $SBATCH_ACCOUNT / sacctmgr) — "
...
def remote_account_probe() -> str:
    ...
    (default `myaccounts`), falling back to sacctmgr. Shared by the --ssh serve
    injection and `boxy doctor --ssh`."""
    cmd = config.get_str("site.account_command").strip() or "myaccounts"
...
def remote_accel_probe(partition: str = "") -> str:
    ...
    gpu tokens, proving this site configures GRES at all: clusterd-class sites
...
def local_accel_probe() -> tuple[str, str]:
    ...
    runs FIRST because existence markers lie: clusterb's login node carries a
...
def remote_scheduler_probe() -> str:
    ...
    `sinfo`/`scontrol` wrappers that PROXY to Flux (field report: `clusterb`,
...
def pick_scheduler(available: str, explicit: str | None = None) -> tuple[str | None, str]:
    ...
    Ranking (explicit flag > config > evidence):
      * a live SYSTEM Flux broker (flux-live) -> FLUX. System Flux runs the machine;
                                             ...
                                             track — the clusterb failure). Override
...
def gpu_types_from_gres(sinfo_text: str, partitions: set[str] | None = None) -> list[str]:
    ...
    site that REQUIRES a type (field: clusterd) is unrecoverable with the untyped
...
def resolve_site(args, scheduler_name: str, need_gpu: bool = False) -> tuple[dict, list[str]]:
    ...
    # e.g. sitescratch:1). Other schedulers (Flux) have no license concept, so only an
...
def resolve_license(explicit: str | None) -> tuple[str, str]:
    ...
    filesystem licenses; clustera prints 'Adding filesystem licenses to job: …')."""
    if explicit:
...
MODEL_STORE_LADDER = (
    ...
    '"/sitescratch/users/$USER"',       # the site big scratch (clustera class)
    '"/sitescratch/$USER"',
    '"/pscratch/users/$USER"',
    '"/pscratch/$USER"',
    '"/scratch/users/$USER"',
...
def render_system_card(cluster: str, scheduler: str, accelerator: str, runtime: str,
    ...
    if inv.get("gpu_vram_gb"):                      # measured directly (nvidia-smi/rocm probe)
        vram, vram_note = int(inv["gpu_vram_gb"]), inv.get("vram_note", "measured by the GPU probe")
    else:
...
# gfx architecture -> the part family (rocminfo speaks gfx, the VRAM table
# speaks product names)
...
# ---- one-shot cluster probe: every pre-submission fact in ONE round-trip ---------
#
# The --ssh flows used to pay ~5 sequential WAN round-trips before submitting
# (scheduler, accelerator, partitions, account, model store — each its own
# ssh_capture). The composite probe concatenates the EXISTING probe snippets
# into one script whose tagged sections feed every decision laptop-side; the
# individual probe functions above stay (fallback + their own tests).

_SECTION_RE = re.compile(r"^===BOXY:([A-Z_]+)===$", re.MULTILINE)


def _accel_marker_snippet() -> str:
    """Existence markers for the laptop-side accelerator ladder (one token per
    line; absent markers print nothing). Same signals as remote_accel_probe's
    fallback branch — the DECISION moves laptop-side where it is testable."""
    return (
        'command -v rocm-smi >/dev/null 2>&1 && echo rocm-smi; '
        '[ -d /opt/rocm ] && echo opt-rocm; '
        '[ -e /dev/kfd ] && echo dev-kfd; '
        'command -v nvidia-smi >/dev/null 2>&1 && echo nvidia-smi; '
        '[ -e /proc/driver/nvidia/version ] && echo nvidia-proc; '
        'true')


def cluster_probe(partition: str = "", store_saved: str = "") -> str:
    """ONE login-node script that emits every static cluster fact as tagged
    sections (===BOXY:NAME=== markers). Sections reuse the single-probe
    bodies verbatim, each terminated with `|| true`-style guards so one broken
    tool can't kill the report. Both schedulers' partition/inventory listings
    are emitted (the scheduler verdict is only known laptop-side, after
    parsing SCHED). `partition` scopes GRESPART; `store_saved` keeps the
    model-store pick sticky."""
    import shlex

    pflag = f" -p {shlex.quote(partition)}" if partition else ""
    parts = [
        ("SCHED", remote_scheduler_probe()),
        ("GRESPART", f"sinfo -h{pflag} -o %G 2>/dev/null || true" if partition else "true"),
        ("GRESALL", "sinfo -h -o %G 2>/dev/null || true"),
        ("MARKERS", _accel_marker_snippet()),
        ("GPUHW", gpu_hw_probe()),
        ("INV_SLURM", sinfo_inventory_probe() + " || true"),
        ("INV_FLUX", flux_inventory_probe() + " || true"),
        ("PARTS_SLURM", remote_partition_probe("slurm")),
        ("PARTS_FLUX", remote_partition_probe("flux")),
        ("ACCOUNT", remote_account_probe()),
        ("STORE", model_store_probe(store_saved)),
        ("RUNTIME", runtime_probe() + "; true"),
        ("IDENT", 'hostname -s 2>/dev/null; printf "%s\\n" "$HOME"'),
    ]
    body = "; ".join(f"echo '===BOXY:{name}==='; {{ {snippet} ; }} 2>/dev/null"
                     for name, snippet in parts)
    return body + "; echo '===BOXY:END==='"


def split_cluster_report(out: str) -> dict[str, str]:
    """Tagged sections -> {NAME: body}. Anything before the first marker (login
    banners — field: government-notice text) is discarded; a report with NO
    markers returns {} (the caller falls back to the individual probes)."""
    sections: dict[str, str] = {}
    matches = list(_SECTION_RE.finditer(out or ""))
    for i, m in enumerate(matches):
        name = m.group(1)
        if name == "END":
            continue
        end = matches[i + 1].start() if i + 1 < len(matches) else len(out)
        sections[name] = out[m.end():end].strip("\n")
    return sections


def _gres_accel(text: str) -> str:
    """The GRES-token classifier from remote_accel_probe, as a pure function:
    'rocm' | 'cuda' | 'gpu' (typeless GPU present) | '' (no gpu tokens)."""
    low = (text or "").lower()
    if re.search(r"mi\d|amd|gfx\d", low):
        return "rocm"
    if re.search(r"h100|h200|a100|v100|gh200|b200|l4|t4|p100|k80|rtx|tesla|nvidia", low):
        return "cuda"
    if "gpu" in low:
        return "gpu"
    return ""
...
def accel_from_report(sections: dict[str, str], partition: str = "") -> tuple[str, str]:
    """(accelerator|'nogpu'|'', why) — the remote_accel_probe ladder computed
    laptop-side from the composite report. Order: partition-scoped GRES (with
    the nogpu corroboration rule: scoped silence is a verdict ONLY when the
    cluster-wide GRES proves the site types its GPUs), cluster-wide GRES, the
    FUNCTIONAL module-probe output (error prose rejected by parse_gpu_hw),
    then the existence markers (rocm before nvidia — a stray broken nvidia-smi
    on an AMD login node must not win)."""
    gres_all = _gres_accel(sections.get("GRESALL", ""))
    if partition:
        scoped = _gres_accel(sections.get("GRESPART", ""))
        if scoped in ("rocm", "cuda"):
            return scoped, f"partition {partition} GRES"
        if scoped == "" and gres_all in ("rocm", "cuda", "gpu"):
            return "nogpu", f"partition {partition} advertises no GPU GRES"
        # typeless scoped GRES ('gpu') falls through to the wider signals
    if gres_all in ("rocm", "cuda"):
        return gres_all, "cluster GRES types"
    gpu_type, _vram, _count = parse_gpu_hw(sections.get("GPUHW", ""))
    hw = accel_from_gpu_type(gpu_type)
    if hw:
        return hw, f"hardware probe after module load ({gpu_type})"
    markers = set((sections.get("MARKERS") or "").split())
    if markers & {"rocm-smi", "opt-rocm", "dev-kfd"}:
        return "rocm", "ROCm userland/driver markers"
    if markers & {"nvidia-smi", "nvidia-proc"}:
        return "cuda", "NVIDIA userland/driver markers"
    return "", ""


@dataclass
class ClusterFacts:
    """One composite probe's parsed report. Thin by design: raw sections plus
    accessors that dispatch to the EXISTING single-probe parsers, so composite
    and fallback paths can never disagree on interpretation."""

    sections: dict[str, str]
    partition: str = ""
    seconds: float = 0.0

    def sched_avail(self) -> str:
        return self.sections.get("SCHED", "")

    def accel(self) -> tuple[str, str]:
        return accel_from_report(self.sections, self.partition)

    def partitions_out(self, scheduler_name: str) -> str:
        return self.sections.get("PARTS_FLUX" if scheduler_name == "flux" else "PARTS_SLURM", "")

    def account_out(self) -> str:
        return self.sections.get("ACCOUNT", "")

    def store_out(self) -> str:
        return self.sections.get("STORE", "")

    def runtimes(self) -> list[str]:
        return [r for r in (self.sections.get("RUNTIME") or "").split() if r]

    def inventory(self, scheduler_name: str) -> dict:
        if scheduler_name == "flux":
            return parse_flux_inventory(self.sections.get("INV_FLUX", ""))
        return parse_sinfo_inventory(self.sections.get("INV_SLURM", ""))

    def gpu_hw(self) -> tuple[str, int, int]:
        return parse_gpu_hw(self.sections.get("GPUHW", ""))

    def hostname(self) -> str:
        lines = [ln.strip() for ln in (self.sections.get("IDENT") or "").splitlines() if ln.strip()]
...
def parse_cluster_report(out: str, partition: str = "",
                         seconds: float = 0.0) -> "ClusterFacts | None":
    """The composite probe's stdout -> ClusterFacts, or None when the output
    carries no ===BOXY:...=== markers at all (transport/shell failure — the
    caller falls back to the individual probes)."""
    sections = split_cluster_report(out)
    if not sections:
        return None
    return ClusterFacts(sections=sections, partition=partition, seconds=seconds)
