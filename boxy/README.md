...
#   auto: account: ab110003 (via myaccounts)
...
- **site discovery** fills `--account` from `myaccounts` / `$SBATCH_ACCOUNT` /
  `sacctmgr`, plus partition/time. When `myaccounts` lists **several charge accounts
  (ACCOUNT_IDs)** and you didn't name one, boxy shows an **interactive picker** on a
  terminal so you choose which the job charges to (remembered per cluster,
  validated against the live list); bypass it with `--account`, `ACCOUNT_ID=fy…`, or
  ...
  Slurm **licenses** can be requested with `--license sitescratch:1` (or
...
boxy serve hf://org/model-GGUF/file.gguf --scheduler slurm --gpus 1 \
    --partition=short --account=myacct --license=sitescratch:1
...
#   boxy generate system --ssh clusterc        # probes sinfo/flux: nodes, GPUs/node,
#                                            # GPU type->VRAM, CPUs, memory, scratch
#                                            # storage -> ~/.config/boxy/cards/systems/clusterc.toml
...
# Then `boxy serve meta-llama/Llama-3.3-70B-Instruct --ssh clusterc` requests
...
boxy serve <model> --ssh clustera --nodes 2 --gpus 4          # explicit TP=GPUs/node, PP=nodes
...
# a big SHARED scratch FS ($SCRATCH, /sitescratch, /pscratch, /scratch — first
...
#   export BOXY_MODEL_DIR=/sitescratch/users/$USER/boxy    # (or config storage.model_dir)
