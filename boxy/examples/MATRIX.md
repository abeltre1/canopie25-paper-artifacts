...
`### Running Command` below is REAL boxy output (generated by dry-run, not
hand-written; volume paths show as $PWD/... for the generating machine, and
pulled-model paths as the dry-run placeholder /path/to/model). The E2E steps
are identical for every combo:
...
and runs on CPU anywhere; the vLLM box (`vllm`, port 8000) expects staged
weights in `${MODELS_DIR}` and GPUs (cluster-class). Scheduler notes: slurm/
flux commands run from a login node; inside an existing allocation use a
location with `scheduler = "none"`. Apptainer serve is foreground: stop with
Ctrl-C or `scancel`/`flux cancel`.
...
```
### Running Command:
    flux run -N1 --gpus-per-node=4 podman run --rm --name=qwen-gguf --network=host --ipc=host --label=boxy.box=qwen-gguf --volume=/path/to/model:/mnt/models/model:ro --group-add=video --cap-add=SYS_PTRACE --device /dev/kfd --device /dev/dri --security-opt seccomp=unconfined --env OMP_NUM_THREADS=1 --env HF_HUB_OFFLINE=1 --env TRANSFORMERS_OFFLINE=1 --env HF_DATASETS_OFFLINE=1 --env HF_HUB_DISABLE_TELEMETRY=1 --env VLLM_NO_USAGE_STATS=1 --env DO_NOT_TRACK=1 --env HF_HUB_ENABLE_HF_TRANSFER=0 ghcr.io/ggml-org/llama.cpp:server -m /mnt/models/model --host 0.0.0.0 --port 8090
```

### llama.cpp + apptainer + flux  (`eldorado.toml`)

```
### Prepare: apptainer build --force qwen-gguf-rocm.sif docker://ghcr.io/ggml-org/llama.cpp:server
### Running Command:
    flux run -N2 --gpus-per-node=4 bash -lc 'module load rocm/6.4.0 && exec apptainer run --fakeroot --writable-tmpfs --cleanenv --no-home --bind /path/to/model:/mnt/models/model:ro --env HF_HOME=/root/.cache/huggingface --rocm --env OMP_NUM_THREADS=1 --env HF_HUB_OFFLINE=1 --env TRANSFORMERS_OFFLINE=1 --env HF_DATASETS_OFFLINE=1 --env HF_HUB_DISABLE_TELEMETRY=1 --env VLLM_NO_USAGE_STATS=1 --env DO_NOT_TRACK=1 --env HF_HUB_ENABLE_HF_TRANSFER=0 qwen-gguf-rocm.sif -m /mnt/models/model --host 0.0.0.0 --port 8090 --gpu-memory-utilization 0.7'
```
...
```
### Running Command:
    flux run -N1 --gpus-per-node=4 docker run --rm --name=qwen-gguf --network=host --ipc=host --label=boxy.box=qwen-gguf --volume=/path/to/model:/mnt/models/model:ro --group-add=video --cap-add=SYS_PTRACE --device /dev/kfd --device /dev/dri --security-opt seccomp=unconfined --env OMP_NUM_THREADS=1 --env HF_HUB_OFFLINE=1 --env TRANSFORMERS_OFFLINE=1 --env HF_DATASETS_OFFLINE=1 --env HF_HUB_DISABLE_TELEMETRY=1 --env VLLM_NO_USAGE_STATS=1 --env DO_NOT_TRACK=1 --env HF_HUB_ENABLE_HF_TRANSFER=0 ghcr.io/ggml-org/llama.cpp:server -m /mnt/models/model --host 0.0.0.0 --port 8090
```
