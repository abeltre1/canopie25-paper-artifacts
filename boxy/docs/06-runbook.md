...
| Podman **CUDA** serve (vLLM) | **G** → **P** | needs CLUSTERA-class node |
...
```bash
# one-shot spelling:
boxy serve <model> --scheduler slurm --gpus 4 --ssh user1@clusterB-login1.example.com
# set-and-forget spelling (then EVERY boxy command is remote, verbatim):
export BOXY_SSH_HOST=user1@clusterB-login1.example.com
boxy serve <model> --scheduler slurm --gpus 4
boxy list        # runs on the cluster
boxy stop <name> # runs on the cluster
# profile spelling: put `remote = "user1@clusterB-login1.example.com"` in [location].
```
...
boxy doctor --ssh user1@clusterB.example.com   # audit the CLUSTER (no boxy needed there)
```
...
```bash
boxy open <inst> --ssh user1@clusterA --port 8090 --share nemotron
...
From a checkout (to run the suite or use `boxy wheels`, which needs an
editable install):
...
pytest -q            # EXPECT: all green; suites needing an absent optional
                     # extra (sky/boto3/matplotlib) or a live daemon SKIP
...
## 3. Slurm + Podman + CUDA cluster (CLUSTERA-class)
...
boxy serve hf://TheBloke/TinyLlama-1.1B-Chat-v1.0-GGUF/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf \
  ...
  --partition=short --account=fyNNNNNN --license=sitescratch:1
# EXPECT:
