...
the same validated configurations, mapped onto **clustera** (4x H100 80GB/node),
**clusterc** (4x H200-class 141GB/node) and **clusterb** (4x MI300A 128GB/node),
...
|---|---|---|---|---|
| Nano 30B-A3B **FP8** | ~30 GB | H100-class+ | 0.11.2 | any — 1 GPU (start here on clusterb) |
| Nano 30B-A3B BF16 | ~60 GB | H100-class+ | 0.11.2 | any — 1 GPU |
| Nano 30B-A3B NVFP4 | ~18 GB | Blackwell native; Hopper via Marlin | 0.11.2 | clusterc/clustera (emulated on clusterb) |
| Super 120B-A12B **FP8** | ~120 GB | H100-class+ | 0.17.1 | clusterc 1xGPU-node / clusterb 2 GPUs |
| Super 120B-A12B BF16 | ~240 GB | H100-class+ | 0.17.1 | clusterc 1 node / clusterb 1 node |
| Ultra 550B-A55B NVFP4 | ~300 GB | 8x H100 (TP1xDP8) / 4-8x Blackwell | **0.22.0 pinned** | clusterc 1 node x4 |
| Ultra 550B-A55B BF16 | ~1.1 TB | 16x H100 / 8x H200 / 8x B200 | **0.22.0 pinned** | clusterc or clusterb, 3-node Ray |
...
boxy serve nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-FP8 --ssh clusterb
boxy serve nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-FP8 --ssh clusterc

# Super — the 120B midpoint:
boxy serve nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-FP8 --ssh clusterb   # 2x MI300A
boxy serve nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-FP8 --ssh clusterc    # 1 node

# Ultra NVFP4 — clusterc is its best home here (1 node x4 H200-class):
boxy serve nvidia/NVIDIA-Nemotron-3-Ultra-550B-A55B-NVFP4 --ssh clusterc
# on clustera (4x80GB nodes) the solver spans 2 nodes as one Ray instance (TP4xPP2);
# the cookbook's own H100 shape is DP8 — replicate that with:
#   boxy serve ...NVFP4 --ssh clustera --replicas 8 --gpus-per-replica 1

# Ultra BF16 — a 3-node Ray instance, automatically:
boxy serve nvidia/NVIDIA-Nemotron-3-Ultra-550B-A55B-BF16 --ssh clusterc
```
...
```bash
boxy bench --ssh clusterc        # TTFT p50/p99, ITL p99, TPOT, E2E, tokens/s
```

## 4. Operate

```bash
boxy list --ssh clusterc         # what's live, where
boxy logs <name> --ssh clusterc  # engine log (root-cause extraction on failure)
boxy stop <name> --ssh clusterc  # kill switch;  boxy stop --all sweeps
boxy clean --ssh clusterc        # sweep finished-job records/scripts/logs
```

## 5. When it breaks (expected first-mover territory on clusterb)
