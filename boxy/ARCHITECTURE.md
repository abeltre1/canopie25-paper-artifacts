...
flowchart TB
    ...
    subgraph R["resolver — turns a name into a plan (cards.py · site.py · resolve.py)"]
        ...
        ST["site resolver<br/>account (mywcid/sacctmgr)<br/>partition · time"]
...
sequenceDiagram
    ...
    CLI->>Site: account? (mywcid → env → sacctmgr)
    Site-->>CLI: fy260064  (auto: account …)
    CLI->>Sched: batch_script(directives, inner serve)
    Sched-->>CLI: #SBATCH --gpus-per-node=4 --account=fy260064 …
...
flowchart LR
    ...
    subgraph Reg["registries + site"]
        ...
        r2["hides: account discovery<br/>(mywcid/sacctmgr)"]
    end
...
| site | `site.py` | account/partition/time discovery | mywcid, env, sacctmgr | 1 probe fn |
