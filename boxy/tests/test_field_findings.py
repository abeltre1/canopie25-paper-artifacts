...
def test_ssl_remedy_names_missing_interceptor_ca_after_os_automerge(monkeypatch):
    """Compute-node case (site, 2026-07): SSL_CERT_FILE was unset, so boxy
...
def test_finding13d_unset_ssl_cert_file_auto_merges_os_trust_store(monkeypatch, tmp_path, capsys):
    """clusterb 2026-07: certifi lacks the site proxy CA that the OS trust store
...
def test_finding4_workdir_without_volume_warns(clustera, tmp_path):
    box = Box(name="w", image="i", model="m", workdir="/vllm-workspace/models")
    d = deploy.plan_serve(box, clustera, dryrun=True)
    ...
    d2 = deploy.plan_serve(ok, clustera, dryrun=True)
    assert d2.warnings == []
...
def test_finding7_llamacpp_defers_to_image_entrypoint(clustera):
    from boxy.backends import get_backend

    box = Box(name="q", engine="llama.cpp", model="m.gguf", ports=[8090])
    d = deploy.plan_serve(box, clustera, dryrun=True)  # clustera runtime=podman
    ...
    d2 = deploy.plan_serve(box2, clustera, dryrun=True)
    assert "--entrypoint=/app/llama-server" in d2.command
    # apptainer: deferred entrypoint switches exec -> run (SIF runscript)
    cmd = get_backend("apptainer").build_command(box, clustera, ["", "-m", "m.gguf"], {}, [], "cuda")
    assert cmd[:2] == ["apptainer", "run"]
    assert "" not in cmd


def test_finding10_missing_volume_source_warns(clustera, tmp_path):
...
def test_finding11_macos_publishes_ports_instead_of_host_network(monkeypatch, clustera):
    from boxy.backends import get_backend
    ...
    cmd = get_backend("podman").build_command(box, clustera, inner, {}, [], "none")
