...
def test_vllm_only_tuning_never_reaches_llamacpp():
    clusterb = Location.from_toml(EXAMPLES / "locations" / "flux-apptainer-rocm.toml")
    box = Box(name="g", image="i:1", engine="llama.cpp", model="m.gguf")
    cmd = engines.build_llamacpp_serve_cmd(box, clusterb, "/m.gguf")
    assert "--gpu-memory-utilization" not in cmd  # llama-server exits 2 on it
    vbox = Box(name="v", image="i:1", engine="vllm", model="m")
    vcmd = engines.build_vllm_serve_cmd(vbox, clusterb, "m")
...
def _slurm_profile(tmp_path, **extra):
    lines = ['[location]', 'name = "site"', 'scheduler = "slurm"', 'runtime = "podman"',
             'accelerator = "cuda"',
             'scheduler_args = ["--partition=site-default", "--license=sitescratch:1"]',
...
def test_r2_attached_mode_consumes_scheduler_flags(gguf, tmp_path, monkeypatch, capsys):
    ...
    assert "--license=sitescratch:1" in out  # profile args survive too
...
def test_proxy_credentials_are_masked():
    masked = ramalama_shim._mask_credentials("http://user1:S3CR3T@proxy.site.gov:3128")
...
def test_raw_proxy_env_pickup_and_override(monkeypatch):
    ...
    assert ov["no_proxy"] == "localhost,.example.gov"
...
def test_info_net_dns_failure_blames_proxy_not_certs(monkeypatch, capsys):
    ...
    monkeypatch.setattr(ramalama_shim, "active_proxies",
                        lambda: {"https": "http://proxy.example.gov:80"})
    rc = cli._probe_registries()
    out = capsys.readouterr().out
    assert rc == 1
    assert "PROXY host itself did not resolve" in out and "proxy.example.gov" in out
