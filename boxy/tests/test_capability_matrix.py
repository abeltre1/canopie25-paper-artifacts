...
def test_docker_rocm_inherits_podman_device_set(vllm_box, clusterb):
    assert get_backend("docker").gpu_args("rocm") == get_backend("podman").gpu_args("rocm")


def test_podman_mount_options_suffix(vllm_box, clustera):
    cmd = get_backend("podman").build_command(vllm_box, clustera, ["x"], {}, [("/a", "/b", "ro")], "none")
...
def test_resolve_runtime_explicit_and_error(monkeypatch, clusterb):
    assert clusterb.resolve_runtime() == "apptainer"
...
def test_multiple_module_loads_joined(clusterb):
    clusterb.modules = ["rocm/6.4.0", "cray-mpich"]
    cmd = get_scheduler("flux").with_modules(["x"], clusterb)
    assert "module load rocm/6.4.0 && module load cray-mpich" in cmd[2]


# ---------- deploy: model resolution & execute ----------

def test_resolve_model_empty_and_absolute(vllm_box, clustera):
    vllm_box.model = ""
    assert deploy.resolve_model(vllm_box, clustera, dryrun=True) == ("", [])
    vllm_box.model = "/shared/fs/llama.gguf"
    path, mounts = deploy.resolve_model(vllm_box, clustera, dryrun=True)
    assert path == "/mnt/models/llama.gguf"
    assert mounts == [("/shared/fs/llama.gguf", "/mnt/models/llama.gguf", "ro")]


def test_resolve_model_transport_uri_dryrun_via_shim(clustera):
    ...
    path, mounts = deploy.resolve_model(box, clustera, dryrun=True)
...
def test_write_task_yaml_tempfile_mode(vllm_box, clustera):
    path = cloud.write_task_yaml(vllm_box, clustera, port=None, serve=False)
