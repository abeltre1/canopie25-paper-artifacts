...
def test_serve_registry_flag_rewrites_image(capsys):
    ...
    assert "registry.sandia.gov/mirror/vllm/vllm-openai:v0.24.0" in out
    assert "image registry: registry.sandia.gov/mirror" in out  # auto: line
...
def test_apptainer_sif_build_uses_rewritten_registry(vllm_box, eldorado):
    from dataclasses import replace

    from boxy.backends import get_backend

    loc = replace(eldorado, registry="registry.site.gov")
...
def test_sbatch_flux_wrapper_hint():
    # eldorado-class systems ship an sbatch that wraps flux; --scheduler slurm
