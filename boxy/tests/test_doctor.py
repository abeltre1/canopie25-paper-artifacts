...
def test_remote_checks_flag_ghcr_403_and_missing_runtime():
    ...
    assert r["scheduler"].status == doctor.OK and "slurm" in r["scheduler"].detail
...
def test_remote_checks_healthy_cluster_all_ok():
    run = _fake_run({
        ...
        "instance-level": "slurm-bin\nslurm-live\n",   # scheduler probe -> slurm live   # slurm control plane is live
        "myaccounts": "ACCOUNT_ID: ab110003  (Genesis)\n",     # account discoverable here
        "nvidia-smi": "cuda\n",
        "https_proxy": "||\n",
...
def test_remote_checks_report_discovered_account():
    run = _fake_run({
        "podman docker apptainer": "podman\n",
        "instance-level": "slurm-bin\nslurm-live\n",   # scheduler probe -> slurm live   # slurm control plane is live
        # the real myaccounts table: the account is ab110001, not the description id
        "myaccounts": ("      User    Account                  Description     Parent\n"
                   "---------- ---------- ------------------------ ----------\n"
                   "     jdoe   ab110001   103732 system software        nd\n"
                   "     jdoe   ab110003   240928 genesis project        nd\n"),
    ...
    assert "ab110001" in r["account discovery"].detail          # first account, not 103732
    assert "ab110003" in r["account discovery"].detail          # alternative named
...
def test_remote_checks_warn_when_no_account_parses():
    run = _fake_run({
        ...
        "myaccounts": "myaccounts: command not found\n",     # nothing account-shaped
        "ghcr.io/v2": "200",
        "boxy --version": "boxy 0.1.0\n",
    })
    r = {x.name: x for x in doctor.remote_checks(run)}
...
def test_cmd_doctor_remote_audit_no_cluster_boxy(monkeypatch, capsys):
    ...
    monkeypatch.setattr(remote, "resolve_target", lambda args: "user1@clustera.example.gov")
    ...
    rc = main(["doctor", "--ssh", "user1@clustera.example.gov"])
    out = capsys.readouterr().out
