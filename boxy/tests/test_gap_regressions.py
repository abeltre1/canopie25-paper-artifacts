...
def test_gap4_apptainer_honors_ro_mount_option(vllm_box, clusterb):
    mounts = [("/host/models", "/models", "ro")]
    cmd = get_backend("apptainer").build_command(vllm_box, clusterb, ["x"], {}, mounts, "rocm")
    i = cmd.index("--bind")
...
def test_gap7_default_image_resolved_from_engine_and_accelerator(clusterb):
    box = Box(name="noimg", engine="vllm", model="m")  # no image
    d = deploy.plan_serve(box, clusterb, dryrun=True)
    ...
    d2 = deploy.plan_serve(llbox, clusterb, dryrun=True)
...
def test_gap8b_container_label_present(vllm_box, clustera):
    cmd = get_backend("podman").build_command(vllm_box, clustera, ["x"], {}, [], "cuda")
