...
command shape for the paper's ClusterB (flux+apptainer+rocm) and CLUSTERA
...
def test_plan_serve_clusterb_end_to_end(vllm_box, clusterb):
    d = deploy.plan_serve(vllm_box, clusterb, dryrun=True)
    cmd = d.command
...
def test_plan_serve_clustera_end_to_end(vllm_box, clustera):
    d = deploy.plan_serve(vllm_box, clustera, dryrun=True)
    cmd = d.command
...
def test_user_args_never_overridden(vllm_box, clusterb):
    d = deploy.plan_serve(vllm_box, clusterb, extra_args=["--seed=7", "--tensor-parallel-size=2"], dryrun=True)
    script = d.command[-1]
...
def test_cli_serve_dryrun_examples(capsys):
    # clusterb is a 2-node vLLM location, so it now auto-distributes; --no-distributed
...
def test_ca_bundle_not_propagated_for_bare_site_cert(vllm_box, clustera, tmp_path, monkeypatch):
...
def test_cli_serve_custom_image(capsys):
    ...
    assert rc == 0
    out = capsys.readouterr().out
    assert "localhost/vllm-extra:latest" in out
    assert "vllm/vllm-openai" not in out  # the custom image replaces the default
