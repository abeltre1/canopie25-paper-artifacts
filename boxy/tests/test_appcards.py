...
# condensed from the real clusterd/clustera failure: every fetcher (spack mirror AND
...
spack.error.FetchError: All fetchers failed for spack-stage-osu-micro-benchmarks-7.5.2-frmvtwk7ojga4y4kl5nfrc5bamxwrrz5
        https://mirror.spack.io/_source-cache/archive/{DIGEST[:2]}/{DIGEST}.tar.gz: DetailedHTTPError: GET http://block-message.ca.example.gov/block-page.html?url=https%3a%2f%2fmirror%2espack%2eio%2f_source-cache%2farchive%2f{DIGEST[:2]}%2f{DIGEST}%2etar%2egz&reason=Not+allowed returned 403: Forbidden
    https://mvapich.cse.ohio-state.edu/download/mvapich/osu-micro-benchmarks-7.5.2.tar.gz: DetailedHTTPError: GET http://block-message.ca.example.gov/block-page.html?url=https%3a%2f%2fmvapich%2ecse%2eohio%2dstate%2eedu%2fdownload%2fmvapich%2fosu-micro-benchmarks-7.5.2%2etar%2egz&reason=Not+allowed returned 403: Forbidden
...
def test_spack_source_heal_stages_verified_archive(monkeypatch, capfd):
    ...
    ok = cli._maybe_spack_source_heal("user@clustera", "clustera", ZSCALER_LOG, "/home/u/mir",
...
def test_spack_source_heal_rejects_sha_mismatch(monkeypatch, capfd):
    ...
    ok = cli._maybe_spack_source_heal("user@clustera", "clustera", ZSCALER_LOG, "/m",
...
def test_stage_source_pushes_at_its_own_digest(tmp_path, monkeypatch, capfd):
    ...
    assert cli._stage_source_file(ns, "user@clustera", "clustera", "/m") is True
...
def test_render_exports_proxy_env_before_spack():
    card = appcards.find_card("osu-benchmarks")
    text = appcards.render_app_script(card, "slurm", "app-osu", "/tmp/x.log", [],
                                      proxy_env={"https_proxy": "http://proxy.example.gov:80"})
    assert "export https_proxy=http://proxy.example.gov:80" in text
...
srun: error: clusterc1: task 0: Exited with exit code 14
"""
...
def test_cli_adhoc_image_with_port_becomes_service(ssh_cluster, capfd, monkeypatch):
    ...
    rc = main(["app", "--image", "docker.io/traefik/whoami:latest", "--port", "8080:80",
               "--env", "WHOAMI_NAME=boxy", "--ssh", "user@clustera", "--dryrun"])
...
def test_flux_mcp_ported_to_a_service_card(ssh_cluster, capfd, monkeypatch):
    ...
    rc = main(["app", "flux-mcp", "--ssh", "user@clustera", "--dryrun"])
    cap = capfd.readouterr()
    assert rc == 0
...
def test_service_follow_opens_local_tunnel_at_ready(monkeypatch, tmp_path, capfd):
    ...
    out = capfd.readouterr().out
    assert rc == 0
    assert forwards == [("user@clusterc", 8080, "nid00123", 8080)]   # the tunnel WAS opened
...
def test_service_ready_tolerates_login_banner(monkeypatch, tmp_path, capfd):
    # FIELD (clusterc): the login node prints a banner on EVERY ssh command, so the
