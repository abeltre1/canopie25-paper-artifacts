...
def test_derive_parallelism_geometry():
    ...
    assert distributed.derive_parallelism(Resources(nodes=2, gpus_per_node=4)) == (4, 2, 8)
    assert distributed.derive_parallelism(Resources(nodes=4, gpus_per_node=8)) == (8, 4, 32)
    assert distributed.derive_parallelism(Resources(nodes=1, gpus_per_node=2)) == (2, 1, 2)
...
def test_worker_launch_prefix_slurm():
    ...
    assert distributed.worker_launch_prefix("slurm", "node0", 2) == [
        "srun", "--nodes=1", "--ntasks=1", "--ntasks-per-node=1", "--exclude", "node0"]
...
def _vllm_box(**args) -> Box:
    return Box(name="vllm", engine="vllm", image="vllm/vllm-openai:v0.9.1",
               entrypoint="vllm", ports=[8000], args=args)


def _loc() -> Location:
    return Location(name="t", scheduler="slurm", accelerator="cuda", runtime="podman",
                    resources=Resources(nodes=2, gpus_per_node=4), staging=Staging(models_dir="./models"))


def test_vllm_serve_cmd_tacks_on_parallelism():
    cmd = engines.build_vllm_serve_cmd(_vllm_box(), _loc(), "/mnt/models/m", parallelism=(4, 2))
...
def test_user_tensor_parallel_size_wins_over_derived():
    ...
    cmd = engines.build_vllm_serve_cmd(_vllm_box(tensor_parallel_size=8), _loc(),
                                       "/mnt/models/m", parallelism=(4, 2))
...
def test_llamacpp_ignores_parallelism():
    box = Box(name="lcpp", engine="llama.cpp", image="ghcr.io/ggml-org/llama.cpp:server",
              entrypoint="", ports=[8090])
...
def test_flux_resource_directives_accept_distributed_flag():
    ...
    loc = Location(name="t", scheduler="flux", accelerator="rocm", runtime="apptainer",
                   resources=Resources(nodes=2, gpus_per_node=4))
    assert (get_scheduler("flux").resource_directives(loc, distributed=True)
...
def _inner_args(**over):
    base = dict(location=None, models_dir=None, nodes=None, gpus=None, distributed=None,
                engine=None, image=None, runtime=None, accelerator=None, port=None, args=None)
    base.update(over)
...
def test_inner_serve_command_forwards_geometry_and_distributed():
    ...
    inner = _inner_serve_command(_inner_args(nodes=2, gpus=4, distributed=True), "m", "vllm")
...
def test_inner_serve_command_no_distributed_forwarded():
    ...
    toks = shlex.split(_inner_serve_command(_inner_args(nodes=2, gpus=4, distributed=False), "m", "vllm"))
...
def test_plan_serve_distributed_head_and_worker():
    box = _vllm_box()
    box = Box(name="vllm", engine="vllm", image="vllm/vllm-openai:v0.9.1",
              entrypoint="vllm", model="models/llama", ports=[8000])
    from boxy import deploy

    dep = deploy.plan_serve(box, _loc(), port=8000, dryrun=True,
                            distributed=True, head_ip="192.0.2.7")
...
def test_plan_serve_distributed_needs_gpu_count():
    box = Box(name="vllm", engine="vllm", image="vllm/vllm-openai:v0.9.1",
              entrypoint="vllm", model="models/llama", ports=[8000])
    loc = Location(name="t", scheduler="slurm", accelerator="cuda", runtime="podman",
