...
RELAY_URL = "https://relay-boxy.apps.ocp.example.gov"
...
def test_apps_domain_strips_relay_label():
    assert relay_mod.apps_domain_from_url(RELAY_URL) == "apps.ocp.example.gov"
...
def test_relay_manifest_shape_and_security():
    ...
    docs = [d for d in yaml.safe_load_all(
        relay_mod.emit_relay_manifest("relay-boxy.apps.ocp.example.gov", "boxy-relay",
...
def test_cli_generate_relay(capsys):
    rc = main(["generate", "relay", "--host", "relay-boxy.apps.ocp.example.gov",
               "--auth", "boxy:pw"])
...
def test_cli_generate_relay_uses_configured_apps_domain(capsys, monkeypatch):
    ...
    # all: boxy-relay.apps.ocp.example.gov — the demo path.
    monkeypatch.setenv("BOXY_APPS_DOMAIN", "apps.ocp.example.gov")
    rc = main(["generate", "relay", "--auth", "boxy:pw"])
    cap = capsys.readouterr()
    assert rc == 0
    assert "auto: relay host: boxy-relay.apps.ocp.example.gov (via config relay.apps_domain)" in cap.err
