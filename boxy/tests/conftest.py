...
def _isolate_config(monkeypatch, tmp_path):
    ...
    # network.proxy ships a site default (proxy.example.gov); blank it so goldens
    # stay site-agnostic. Proxy tests pass --proxy explicitly.
    monkeypatch.setenv("BOXY_PROXY", "")
    # relay.apps_domain ships a site default (apps.ocp.example.gov); blank it
    ...
    # ditto remote account injection (a dev box may carry sacctmgr/myaccounts,
    ...
    # the interactive ACCOUNT_ID picker is a TTY feature; keep it OFF for the suite so no
    ...
    # LOCALLY, and a root runner could really create /sitescratch). Pin the store
    # per test; the discovery tests blank this to opt back in.
    monkeypatch.setenv("BOXY_MODEL_DIR", str(tmp_path / "model-store"))
    # site.license defaults to sitescratch:1 (the site); neutralize it for the broad
