...
def test_podman_cuda(vllm_box, hops):
    cmd = get_backend("podman").build_command(vllm_box, hops, INNER, {}, MOUNTS, "cuda")
...
def test_podman_rocm(vllm_box, eldorado):
    cmd = get_backend("podman").build_command(vllm_box, eldorado, INNER, {}, MOUNTS, "rocm")
...
def test_apptainer_rocm(vllm_box, eldorado):
    backend = get_backend("apptainer")
    cmd = backend.build_command(vllm_box, eldorado, INNER, {}, MOUNTS, "rocm")
...
def test_apptainer_cuda_uses_nv(vllm_box, hops):
    cmd = get_backend("apptainer").build_command(vllm_box, hops, INNER, {}, MOUNTS, "cuda")
    assert "--nv" in cmd and "--rocm" not in cmd
    assert "vllm-cuda.sif" in cmd


def test_apptainer_prepare_builds_sif(vllm_box, eldorado):
    prepare = get_backend("apptainer").prepare(vllm_box, eldorado)
...
def test_docker_cuda_uses_gpus_all(vllm_box, hops):
    cmd = get_backend("docker").build_command(vllm_box, hops, INNER, {}, MOUNTS, "cuda")
    i = cmd.index("--gpus")
    assert cmd[i + 1] == "all"


def test_env_injection(vllm_box, eldorado):
    env = envs.build_env(vllm_box.env, "rocm", offline=True)
    cmd = get_backend("apptainer").build_command(vllm_box, eldorado, INNER, env, MOUNTS, "rocm")
    joined = " ".join(cmd)
...
def test_registry_prefix(vllm_box, hops):
    hops.registry = "registry.example.com/"
    cmd = get_backend("podman").build_command(vllm_box, hops, INNER, {}, MOUNTS, "cuda")
