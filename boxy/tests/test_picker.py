"""Unit tests for the interactive ACCOUNT_ID/account picker (src/boxy/picker.py)."""
...
ROWS = [("ab110001", "system software"), ("ab110002", "advanced computing"),
        ("ab110003", "ml research")]
...
def test_explicit_bypasses_and_returns_none():
    ...
    assert picker.choose_account(ROWS, explicit="ab110002", mode="always") == (None, "")


def test_explicit_not_in_list_warns_but_proceeds(capsys):
    pick, note = picker.choose_account(ROWS, explicit="fy999", mode="always")
    assert pick is None
    assert "not among your myaccounts accounts" in capsys.readouterr().err
...
def test_non_interactive_takes_first_of_many():
    ...
    assert pick == "ab110001"
    assert "first of 3" in note and "--account or export ACCOUNT_ID" in note


def test_non_interactive_prefers_a_valid_remembered_default():
    pick, note = picker.choose_account(ROWS, remembered="ab110003", mode="never")
    assert pick == "ab110003" and "remembered default" in note
...
def test_interactive_pick_by_number(jobsdir, monkeypatch, capsys):
    ...
    pick, note = picker.choose_account(ROWS, mode="always", where="clustera")
    assert pick == "ab110002"
    assert "you picked 2 of 3" in note
    err = capsys.readouterr().err
    assert "Select a charge account" in err and "2) ab110002" in err
    # the pick is remembered per-target for next time
    assert picker.recall("clustera") == "ab110002"


def test_interactive_enter_reuses_remembered_default(jobsdir, monkeypatch):
    _answer(monkeypatch, "")                       # bare Enter
    pick, _ = picker.choose_account(ROWS, remembered="ab110003", mode="always", where="clustera")
    assert pick == "ab110003"
...
def test_interactive_eof_falls_back_to_first(jobsdir, monkeypatch):
    ...
    pick, _ = picker.choose_account(ROWS, mode="always")
    assert pick == "ab110001"                       # default index 0, no traceback
...
def test_remember_recall_roundtrip_is_per_target(jobsdir):
    picker.remember("fy1", where="clustera")
    picker.remember("fy2", where="clusterb")
    assert picker.recall("clustera") == "fy1"
    assert picker.recall("clusterb") == "fy2"
...
def test_recall_survives_a_missing_jobs_dir(jobsdir):
    ...
    picker.remember("fy1", where="clustera")
    assert (jobs._dir() / "last_account.clustera").exists()
...
def test_partition_remembered_default_is_partition_scoped(jobsdir):
    picker.remember("batch", where="clustera", kind="partition")
    picker.remember("fy1", where="clustera", kind="account")
    assert picker.recall("clustera", kind="partition") == "batch"
    assert picker.recall("clustera", kind="account") == "fy1"    # separate files
