...
- **Build the bundle for the TARGET's accelerator**: `--accelerator rocm` picks
  the ROCm vLLM image for an MI300 system (cronus/eldorado class), `cuda` for
  H100/H200 (hops class). One bundle per accelerator family.
...
- [ ] **The chisel relay image** (`quay.sandia.gov/ambelt/chisel:1.10.1`) if the
...
- [ ] `network.proxy` default (`proxy.sandia.gov`) is meaningless inside —
