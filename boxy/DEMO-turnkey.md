...
the model card), the charge account (from `myaccounts`), a soonest-start partition
...
(the account table is the field `myaccounts` format).
...
1. detects the live scheduler + resolves the site (`myaccounts` account, `sinfo`
...
> "you should have gotten the account number using `myaccounts` from the HPC system
...
to the *cluster's* boxy, which may predate turnkey — so `myaccounts` never ran and the
...
```console
$ boxy doctor --ssh user1@clustera
boxy doctor — remote audit of user1@clustera (no boxy required on the cluster)

doctor
  container runtime          [OK] podman
  scheduler                  [OK] will submit via: slurm — detected (Slurm is live — sinfo listed partitions)
  account discovery          [OK] ab110001 (also: ab110002, ab110003) — turnkey will place
                                  `--account=ab110001` in the batch script
...
`myaccounts` probe the serve path uses and shows you the account boxy *will* use. If
it WARNs (`no account parsed …`), the raw `myaccounts` output is printed so you can
pass `--account <account_id>` explicitly and file the format.
...
```console
$ boxy serve hf://meta-llama/Llama-3.1-8B-Instruct --ssh user1@clustera
  auto: scheduler: slurm (via detected (Slurm is live — sinfo listed partitions) on clustera)
  auto: partition: gpu,batch (via sinfo on clustera: 2 partitions with GPUs, soonest-start)
  auto: account: ab110001 (via myaccounts on clustera; also: ab110002, ab110003 — placed in the batch script)
...
### Batch script (…/boxy-llama-3.1-8b-instruct.sh):
    ...
    #SBATCH --account=ab110001          <-- from myaccounts, no flag typed
...
### READY  http://clustera-gpu07:8000/v1   (model: …, slurm job 1786916)
###   tunnel: ssh -L 8000:clustera-gpu07:8000 clustera
###   stop:  boxy stop boxy-llama-3.1-8b-instruct
```

> **The GPU request self-heals — without changing a working cluster.** boxy uses
> the proven `--gpus-per-node=N` by default (what works on clustera/clusterb). It does
...
> field report: clusterd) — so if a submit is rejected for the GPU line, boxy
...
> **Choosing your ACCOUNT_ID (charge account).** When `myaccounts` lists several accounts
...
> $ boxy serve hf://meta-llama/Llama-3.1-8B-Instruct --ssh user1@clustera
>   ...
> Select a charge account (ACCOUNT_ID):
>   1) ab110001  system software and tools
>   2) ab110002  common computing environment  [default]
>   3) ab110003  the genesis project
> account [1-3] (Enter = 2): 3
>   auto: account: ab110003 (you picked 3 of 3 from myaccounts on clustera)
> ```
>
> Your pick is remembered per cluster (hit **Enter** next time to reuse it, shown
> as `[default]`), and validated against the live `myaccounts` list so a stale default
> can't charge an account you've lost. Bypass the menu entirely with `--account
> fy…`, `export ACCOUNT_ID=fy…` (handy in scripts/CI), or a pinned `BOXY_ACCOUNT`. The
...
(`server starting on clusterc5 … http://clusterc5:8000/…`), your laptop opens the SSH
...
`http://clusterc5:8000/v1/models` GET went through the corporate proxy). Fallbacks:
...
On a login node directly (no `--ssh`), `myaccounts`/`sinfo` run locally. The
...
So `clusterb` (a Flux system whose slurm shims answer too) is correctly detected
...
```console
$ boxy serve <model> --scheduler slurm --account ab110003 --ssh user1@clustera
```
...
```console
$ boxy serve hf://meta-llama/Llama-3.1-8B-Instruct --scheduler slurm --ssh user1@clustera
  auto: partition: gpu,batch (via sinfo on clustera: 2 partitions with GPUs, soonest-start (most idle: gpu (6 nodes)))
  auto: account: ab110001 (via myaccounts on clustera …)
### Batch script (…):
...
```console
$ boxy serve hf://meta-llama/Llama-3.1-8B-Instruct --ssh user1@clusterb
  auto: scheduler: flux (via detected (Flux broker is live) on clusterb)
  auto: account: ab110001 (via myaccounts on clusterb; …)
### Batch script (…):
    #!/bin/bash
    # flux: --job-name=boxy-llama-3.1-8b-instruct
    # flux: -N1
    # flux: -g1
    # flux: --bank=ab110001            <-- Flux spells 'account' as 'bank'
```
...
|---|---------|--------|-----------------|
| 0 | `boxy doctor --ssh user1@clustera` | `account discovery: OK ab110001 …` | `test_doctor.py::test_remote_checks_report_discovered_account` |
| 1 | `boxy serve <8B> --ssh user1@clustera` (no `--scheduler`) | `auto: scheduler: slurm (via detected (Slurm is live …) on clustera)`, then `#SBATCH --account=ab110001` | `test_turnkey_e2e.py::test_ssh_auto_detects_scheduler_when_flag_absent` |
| 1b | `boxy serve <8B> --scheduler slurm --ssh user1@clustera` | `auto: account: ab110001 (via myaccounts on clustera …)` then `#SBATCH --account=ab110001` | `test_turnkey_e2e.py::test_ssh_probes_myaccounts_on_the_cluster` |
| 2 | on clustera: `BOXY_SCHEDULER=slurm boxy serve <8B>` (no `--scheduler`) | `auto: scheduler: slurm (via config site.scheduler)`, `#SBATCH --account=ab110001` | `test_turnkey_e2e.py::test_login_node_scheduler_from_config` |
| 2b | `boxy serve <8B> --scheduler slurm --partition auto --ssh user1@clustera` | `#SBATCH --partition=<idle-first list>` — starts wherever frees first | `test_turnkey_e2e.py::test_ssh_resolves_partition_auto_to_concrete_list` |
| 2c | `boxy serve <8B> --ssh user1@clustera` (no `--time`) | `auto: time: 30:00 …`, `#SBATCH --time=30:00` | `test_turnkey_e2e.py::test_ssh_injects_default_walltime` |
| 3 | `boxy serve <8B> --ssh user1@clusterb` (Flux auto-detected) | `# flux: --bank=ab110001`, one queue | `test_turnkey_e2e.py::test_login_node_flux_bank_and_single_queue` |
| 4 | `boxy list --ssh user1@clustera` | the job as `RUNNING`, then its endpoint | submit/follow loop in `_serve_submission` |

The account resolution and its placement in the batch script are proven
end-to-end in the sandbox with fake `myaccounts`/`sbatch`/`ssh` shims; steps 1–4
against the live clustera/clusterb systems are the only part that must run on your
...
After this, `boxy doctor --ssh user1@clustera` reports the version instead of
