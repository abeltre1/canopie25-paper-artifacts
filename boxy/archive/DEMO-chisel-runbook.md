...
> Placeholders: `user1@clustera` = your cluster login; `<api-url>` = the OpenShift
...
- **Cluster login (`clustera`):** boxy installed; Slurm; `HF_TOKEN` exported in your
...
```bash
export CLUSTER_LOGIN=user1@clustera
...
echo "apps domain: $APPS"                            # e.g. apps.ocp.example.gov
```
...
boxy serve hf://meta-llama/Llama-3.2-1B-Instruct \
     ...
     --partition=short,batch --account ab110003 --unique \
     --ssh $CLUSTER_LOGIN \
     --share demo
```
Expected output, top to bottom:
```text
### CA      copied your site CA -> user1@clustera:$HOME/.local/share/boxy/store/laptop-ca.crt  (remote SSL_CERT_FILE)
### Remote  user1@clustera  $ boxy serve hf://meta-llama/Llama-3.2-1B-Instruct --scheduler slurm ...
...
| Symptom | Cause / fix |
|---|---|
| No `### READY` | job pending or the pull failed. `boxy logs demo --ssh user1@clustera`. |
...
| pull `401`/gated | `HF_TOKEN` not set on the cluster login shell. `echo 'export HF_TOKEN=hf_…' >> ~/.bashrc` on `clustera`. |
...
1. **One command, from my laptop** — submits a Slurm job on `clustera`, waits for READY,
   tunnels it home.
